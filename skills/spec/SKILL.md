...
### 6. 业务规则与验收契约（BR / AC）
Spec 聚焦于**代码级业务不变量（BR）**与**自动化断言（AC）**，可直接就近内嵌在各模块/接口规格下方，或在独立章节声明：
- **业务规则（BR-##）**：系统在任何时刻都必须严格满足的代码级业务不变量（如校验逻辑、状态转换约束、排序与默认值策略）：
```markdown
- `BR-01`：<明确的单条业务规则或防御约束>
- `BR-02`：<明确的单条业务规则或防御约束>
```
- **验收标准（AC-##）**：**禁止使用字母后缀变体（如 AC-01a）**。每条 AC 必须是可由无上下文 AI 直接转化为测试断言的精准用例（GIVEN 前置条件 + WHEN 触发输入 + THEN 唯一可观察结果）：
```markdown
...
- [ ] 业务规则 BR 与验收用例 AC 编号完整，独立可 PASS/FAIL，无字母后缀编号
